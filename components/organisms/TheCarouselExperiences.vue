...
<template>
  <section class="relative">
    <div
      ref="slider" class="flex flex-row text-white overflow-x-scroll snap-x snap-mandatory no-scrollbar"
      tabindex="0"
    >
      <article
        v-for="(experience, indice) in props.experiences" :key="experience.title"
        v-intersection-observer="[onIntersectionSlide(indice), { root: slider }]" class="px-4 pt-14 pb-[7.5rem] w-screen shrink-0 snap-start"
        :class="experience['background-color']"
      >
    ...
    >
      <button
        v-for="value in Object.keys(props.experiences).length" :key="value"
        ...
        :tabindex="isCurrentSlide(value - 1) ? '-1' : '0'" :disabled="isCurrentSlide(value - 1) ? true : false"
        @click="goTo(value - 1)"
      />
    </div>
  </section>
